...
import { glslTokenPricingForOpenRouterModel } from '@shader-oracle/shared';
import type { Env } from '../env.js';

const OPENROUTER_MODELS_ENDPOINT = 'https://openrouter.ai/api/v1/models';
const MODEL_CACHE_MS = 5 * 60 * 1000;

export const FEATURED_MODELS: OpenRouterModelDto[] = [
...
export const FEATURED_MODEL_IDS = FEATURED_MODELS.map((model) => model.id);
...
function withTokenPricing(model: OpenRouterModelDto): OpenRouterModelDto {
  return {
    ...model,
    glslTokenPricing: glslTokenPricingForOpenRouterModel(model.pricing, model.id),
  };
}
...
function normalizeModel(model: OpenRouterModelPayload): OpenRouterModelDto | undefined {
  ...
  const contextLength =
    typeof model.context_length === 'number' && Number.isFinite(model.context_length)
      ...
      : undefined;
  const normalizedPricing = pricing(model.pricing);

  return withTokenPricing({
    id: model.id,
...
function buildCatalog(models: OpenRouterModelDto[]): OpenRouterModelDto[] {
  ...
  for (const model of FEATURED_MODELS.map(withTokenPricing)) {
...
export class ModelCatalog {
  private cached: { expiresAt: number; response: ModelsResponse } | undefined;

  constructor(
    private readonly env: Env,
    private readonly defaults: { defaultModel: string; providerName: string },
  ) {}
  ...
  async listModels(): Promise<ModelsResponse> {
    ...
    if (this.env.openrouterApiKey) {
      headers.Authorization = `Bearer ${this.env.openrouterApiKey}`;
    }

    const response = (await fetch(OPENROUTER_MODELS_ENDPOINT, { headers }).catch(
      () => undefined,
    )) as FetchResponse | undefined;
    if (!response?.ok) return this.modelsResponse(FEATURED_MODELS.map(withTokenPricing), now);
    ...
    this.cached = {
      ...
      response: this.modelsResponse(models, now),
    };

    return this.cached.response;
  }

  async getModel(modelId: string): Promise<OpenRouterModelDto | undefined> {
    const catalog = await this.listModels();
    return catalog.models.find((model) => model.id === modelId);
  }

  private modelsResponse(models: OpenRouterModelDto[], fetchedAt: number): ModelsResponse {
    return {
